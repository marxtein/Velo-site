...
function renderPapers(){
  ...
  document.getElementById('gdP').innerHTML=flt.map(function(x){return'<div class="card"><div class="hd"><div class="ic">'+x.t[0]+'</div><div><div class="tl">'+x.t.slice(0,60)+(x.t.length>60?'…':'')+'</div><div class="me">👤 '+x.a+' · 📅 '+x.y+' · 📰 '+x.j+'</div></div></div><div class="ds">'+x.h+'</div><div class="ft">🔗 关联代码: <b>'+x.c+'</b>'+(x.l?' · <a href="'+x.l+'" target="_blank">论文链接 →</a>':'')+'</div></div>'}).join('')||'<div style="text-align:center;padding:3rem;color:var(--mu);grid-column:1/-1">没有匹配</div>';
}
